...
class InstrumentedTest {
    ...
    fun addItemRotation() {
        ...
        onView(withId(R.id.URL))
            .perform(typeText(stringToBeTyped), androidx.test.espresso.action.ViewActions.closeSoftKeyboard())
        //trigger error
        ...
        onView(withId(R.id.text_input_layout))
            .check { view, _ -> assert((view as TextInputLayout).error == getResourceString(R.string.invalid_url)) }
    ...
    fun testAddGroupAndDeleteGroupRotation() {
        ...
        openActionBarOverflowOrOptionsMenu(InstrumentationRegistry.getInstrumentation().targetContext)//click on "..."
    ...
    fun rotateMainScreenAndCheckRecyclerPositionRestoring() {
        ...
        onView(withId(R.id.recycler_view_items))
                .perform(RecyclerViewActions.actionOnItemAtPosition<ItemsAdapter.ItemsViewHolder>(10, click()))
        //goto edit screen
        onView(withId(R.id.menu_edit))
                .perform(click())
        //fill the inputs in editscreen
        onView(withId(R.id.name))
                .perform(replaceText(stringToBeTyped), androidx.test.espresso.action.ViewActions.closeSoftKeyboard())
        //save and exit
        onView(withId(R.id.fab_save))
                .perform(click())
        setPortraitOrientation()
    ...
    fun deleteItemRotation() {
        ...
        //goto details
        onView(withId(R.id.recycler_view_items))
                .perform(RecyclerViewActions.actionOnItemAtPosition<ItemsAdapter.ItemsViewHolder>(0, click()))
    ...
    fun testFlowAddGroup_master_detail_editItem_deleteGroup() {
        ...
        onView(withId(R.id.auto_complete_text_view))
                .check { view, _ -> existingGroupName = (view as AutoCompleteTextView).text.toString() }//save existing group name
                .perform(click())
        onView(withText(exampleGroupName))
                .inRoot(RootMatchers.isPlatformPopup())
                .perform(click())
    ...
    fun sortingMode() {
        ...
        onView(withId(R.id.recycler_view_items))
                .perform(RecyclerViewActions.actionOnItemAtPosition<ItemsAdapter.ItemsViewHolder>(10, click()))
        onView(withId(R.id.collapsing_toolbar))
                .check { view, _ -> assert((view as CollapsingToolbarLayout).title.toString() == itemName) }
    }
    ...
    fun searchViewWithRotation() {
        ...
        onView(withId(R.id.recycler_view_items))
                .perform(RecyclerViewActions.actionOnItemAtPosition<ItemsAdapter.ItemsViewHolder>(0, click()))
        onView(withId(R.id.collapsing_toolbar))
                .check { view, _ -> assert((view as CollapsingToolbarLayout).title.toString() == stringToBeTyped) }
        //goto edit screen
        onView(withId(R.id.menu_edit))
                .perform(click())
    ...
    fun swipeToEditWithRotation() {
        ...
        //goto detail for change the name of the item
        onView(withId(R.id.recycler_view_items))
                .perform(RecyclerViewActions.actionOnItemAtPosition<ItemsAdapter.ItemsViewHolder>(0, swipeRight()))
        onView(withId(R.id.name))
                .perform(replaceText(stringToBeTyped))
        onView(withId(R.id.fab_save))
                .perform(click())
