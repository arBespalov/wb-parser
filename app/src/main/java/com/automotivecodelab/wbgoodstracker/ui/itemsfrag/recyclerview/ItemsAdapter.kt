...
import com.automotivecodelab.wbgoodstracker.millisToDays
import com.automotivecodelab.wbgoodstracker.ui.itemsfrag.ItemsViewModel
import com.automotivecodelab.wbgoodstracker.ui.itemsfrag.OpenItemDTO
...
class ItemsAdapter (
    ...
    val sortedList = SortedList<Item>(Item::class.java, object: SortedList.Callback<Item>() {
    ...
    inner class ItemsViewHolder(val recyclerViewItemBinding : RecyclerviewItemBinding) :RecyclerView.ViewHolder(recyclerViewItemBinding.root), ViewHolderWithDetails<String> {

        init {
            this.recyclerViewItemBinding.card.setOnClickListener {
//                val extras = FragmentNavigatorExtras(
//                    this.recyclerViewItemBinding.card to
//                            this.itemView.context.getString(R.string.shared_element_container_detail_fragment))
//                viewModel.openItem(OpenItemDTO(sortedList[bindingAdapterPosition]._id, extras))

                viewModel.openItem(bindingAdapterPosition)
            }
        }

        override fun getItemDetail() = MyItemDetails(bindingAdapterPosition, sortedList.get(bindingAdapterPosition)._id)
    }
    ...
    override fun onBindViewHolder(holder: ItemsViewHolder, position: Int) {
        ...
        tracker?.let {
            if (it.isSelected(sortedList[position]._id)) {
                holder.recyclerViewItemBinding.card.setCardBackgroundColor(MaterialColors.getColor(holder.recyclerViewItemBinding.root, R.attr.colorSecondary))
                //holder.recyclerViewItemBinding.root.isActivated = true
            } else {
                holder.recyclerViewItemBinding.card.setCardBackgroundColor(MaterialColors.getColor(holder.recyclerViewItemBinding.root, R.attr.colorSurface))
