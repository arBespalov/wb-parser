...
class ConfirmDeleteGroupDialogFragment : BottomSheetDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        bottomSheetDialog.findViewById<Button>(R.id.ok)?.setOnClickListener { viewModel.deleteGroup(args.groupName) }
    ...
    private fun setupNavigation() {
        viewModel.taskCompletedEvent.observe(this, EventObserver {//"this" instead of viewLifeCycleOwner because viewLifeCycleOwner for dialog won't be initialized
            val action = ConfirmDeleteGroupDialogFragmentDirections.actionConfirmDeleteGroupDialogFragToItemsFragment()
            navigate(action)
        })
    }
}